...
use super::{key::{verify_puzzle, NodeID, NodeValidator, encrypt_message}, kademlia::Header};


pub struct Signer {}
impl Signer {
pub fn sign_strong_header_req(timestamp: i64, pub_key: &[u8], address: &str, data: Vec<u8>) -> Vec<u8>{
...
pub fn sign_weak_header_req(timestamp: i64, pub_key: &[u8], address: &str) -> Vec<u8>{
    let mut hasher = Sha256::new();
    hasher.update(&timestamp.to_be_bytes());
    hasher.update(address.as_bytes());
...
pub fn validate_strong_req(validator: &NodeValidator, header: &Header, address: &str, data: &[u8]) -> Result<(), &'static str> {
    let node_id = NodeID::from_vec(header.my_id.clone());
    let nonce = header.nonce;
    let signature = validator.decrypt(&header.signature);
    let timestamp = header.timestamp;
